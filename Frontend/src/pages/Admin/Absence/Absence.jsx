...
export default function Absencepage() {
    ...
    const handleAllow = async (AbsenceID) => {
        try {
            ...
            setAbsence(prevState => ({
                ...prevState,
                absences: prevState.absences.filter(absence => absence.id !== AbsenceID)
            }));
    ...
    const handlejustified = async (AbsenceID) => {
        try {
            const token = Cookies.get('XSRF-TOKEN')
            ...
            <div className={styles.absencelist}>
                {is_loading ? <MiniSpinner /> : (
                    <table className={styles.table}>
                        ...
                        <tbody>
                            {absences.map(({ id, date, start_time, end_time, student }) => (
                                <tr className={`${styles.row} ${student.warning_count >= 3 ? styles.red : ''}`} key={id}>
                                    ...
                                    <td className={styles.td}>
                                        <button onClick={() => handlejustified(id)} className={styles.buttons}>Justified</button>
                                        <button
                                            onClick={() => handleAllow(id)}
